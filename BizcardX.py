...
con = mysql.connector.connect(host="localhost", user="root", password="Ashu@123", database="BizcardX")
print(con)
mycursor = con.cursor()
...
icon = Image.open(r"C:/Users/mkolwal/PycharmProjects/new/Title.jpg")
st.set_page_config(page_title= "BizCardX: Extracting Business Card Data with OCR | By Mayank Raj",
                   page_icon= icon,
                   layout= "wide",
                   initial_sidebar_state= "expanded",
                   menu_items={'About': """# This OCR app is created by *Mayank*!"""})
...
if selected == "Home":
    ...
    with col2:
        image_path = Image.open("Title.jpg")
        st.image(image_path, caption="My Local Image", use_column_width=True)
        ...
        with col2:
            ...
            if st.button("Upload to Database"):
                for row in data.itertuples(index=False):
                    insert_query = "insert into card_data(company_name, card_holder, designation,mobile_number,email," \
                                   "website,area,city,state,pin_code,image) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
...
if selected == "Modify":
    ...
    try:
        with column1:
            ...
            if st.button("Commit changes to DB"):
                ...
                mobile_number=%s,email=%s,website=%s,area=%s,city=%s,state=%s,pin_code=%s WHERE card_holder=%s""", (
    ...
    if st.button("View updated data"):
        mycursor.execute(
            "select company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code from "
            "card_data")
